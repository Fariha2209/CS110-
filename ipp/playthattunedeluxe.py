# Reads sound samples, each characterized by a pitch and a duration, as standard input; and plays the sound using 
# standard audio.
...
# Returns an array of samples for a note superposed from three notes (at pitch, 2 x pitch, and 0.5 x pitch) and having 
# the specified duration.
def _createRichNote(pitch, duration):
    NOTES_ON_SCALE = 12
    CONCERT_A = 440.0
    hz = CONCERT_A * pow(2, pitch / NOTES_ON_SCALE)
...
# Superposes arrays a and b, using respective weights aWeight and bWeight, and returns the superposed array.
